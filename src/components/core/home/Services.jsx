...
const Services = () => {
  return (
    <section>
      ...
      <div className="my-10 sl:my-20 grid gap-10 text-center sl:grid-cols-3 sl:gap-3 sl:text-left">
        {services.map((service) => (
          <div key={service.id}>
            <img
              src={service.img}
              alt=""
              className={`bg-${service.color} px-2 rounded-2xl`}
            />
            <p className="capitalize font-bold text-2xl my-3">
              {service.title}
            </p>
